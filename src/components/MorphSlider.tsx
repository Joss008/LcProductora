...
class MorphEngine {
  ...
  constructor(container: HTMLElement, opts: {
    ...
    this.program = new Program(this.gl, {
      ...
      uniforms: {
        ...
        uProgress: { value: 0 },
        uDir: { value: 1 },
        uMode: { value: TRANSITIONS[(options.transition as string) ?? 'melt'] ?? 0 },
        uIntensity: { value: options.intensity as number },
        uScale: { value: options.scale as number },
        uAberration: { value: options.aberration as number },
        uDrift: { value: options.drift as number },
        uTime: { value: 0 },
        uReduce: { value: opts.reducedMotion ? 1 : 0 },
        uPointer: { value: [0.5, 0.5] },
        uOverlay: { value: hexToRgb(options.overlayColor as string) }
      }
    });
  ...
  syncOptions() {
    ...
    this.program.uniforms.uMode.value = TRANSITIONS[(opts.transition as string) ?? 'melt'] ?? 0;
    this.program.uniforms.uIntensity.value = opts.intensity as number;
    this.program.uniforms.uScale.value = opts.scale as number;
    this.program.uniforms.uAberration.value = opts.aberration as number;
    this.program.uniforms.uDrift.value = opts.drift as number;
    this.program.uniforms.uOverlay.value = hexToRgb(opts.overlayColor as string);
  }
  ...
  goTo(dirOrIndex: number) {
    if (this.animating || this.dragging || this.items.length < 2) return;
    const opts = this.getOptions();
    let dir: number;
    if (Math.abs(dirOrIndex) === 1) {
      dir = dirOrIndex;
    } else {
      dir = dirOrIndex > this.current ? 1 : -1;
    }
    if (!opts.loop) {
    ...
    const duration = this.reducedMotion ? Math.min(opts.duration as number, 0.4) : (opts.duration as number);
  ...
  endDrag() {
    ...
    if (p > 0.4) {
      ...
      this.tween = gsap.to(this.program.uniforms.uProgress, {
        value: 1, duration, ease: 'power2.out',
        onComplete: () => this.commit(target)
      });
    } else {
...
export default function MorphSlider({
  ...
  useEffect(() => {
    ...
    if (!engine || startIndex === undefined) return;
    if (startIndex === engine.current) return;
    if (engine.animating) return;
    
    const dir = startIndex > engine.current ? 1 : -1;
    engine.goTo(dir);
  }, [startIndex]);
